...
import org.voxity.dialer.ui.theme.DialerTheme
import kotlinx.coroutines.*
import org.koin.android.ext.android.get
import org.voxity.dialer.components.SaveContactDialog
...
class MainActivity : ComponentActivity() {
    ...
    private var showSaveContactDialog by mutableStateOf(false)
        ...
        setContent {
            DialerTheme {
                ...
                if (showSaveContactDialog) {
                    SaveContactDialog(
                        phoneNumber = activeCallState.callState.phoneNumber,
                        isVisible = showSaveContactDialog,
                        onSave = { contactName ->
                            scope.launch {
                                val result = get<CallUseCases>().saveContact(
                                    contactName,
                                    activeCallState.callState.phoneNumber
                                )
                                when (result) {
    ...
    private fun onSaveContact() {
        scope.launch {
            val state = activeCallState.callState
    ...
    private fun createActiveCallCallbacks() = object : ActiveCallScreenCallbacks {
        override fun onAnswerCall() { scope.launch { get<CallUseCases>().answerCall() } }
        override fun onRejectCall() { scope.launch { get<CallUseCases>().rejectCall() } }
        override fun onEndCall() { scope.launch { get<CallUseCases>().endCall() } }
        override fun onHoldCall() { scope.launch { get<CallUseCases>().holdCall() } }
        override fun onUnholdCall() { scope.launch { get<CallUseCases>().unholdCall() } }
        override fun onMuteCall(muted: Boolean) { scope.launch { get<CallUseCases>().muteCall(muted) } }
    ...
    override fun onBackPressed() {
        ...
        when {
            callState.isConnecting || callState.isActive -> {
                // Move app to background but keep call active
                moveTaskToBack(true)

                // Show notification for returning to call
                showOngoingCallNotification()
            }
            callState.isRinging -> {
                Toast.makeText(this, "Please answer or reject the call first", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun showOngoingCallNotification() {
        val notificationManager = get<CallNotificationManager>()
        val callState = activeCallState.callState

        // Create a persistent notification
        notificationManager.showOngoingCallNotification(
            callerName = callState.contactName.ifEmpty { callState.phoneNumber },
